...
new_message_encoded = []
list_key_public = list(str(e))
message_encoded = [ord(c) for c in message]
i=0

for number in message_encoded:
    index = int(list_key_public[i%len(list_key_public)])
    new_message_encoded.append(number + index)
    i+=1

#Crypt
# (m**e) mod n = c
crypt_message =  [pow(c, e, n) for c in new_message_encoded]
print(crypt_message)
...
for number in decrypt_message:
    index = int(list_key_public[i%len(list_key_public)])
    message_decoded.append(number - index)
    i+=1

new_message = "".join(chr(ch) for ch in message_decoded)
