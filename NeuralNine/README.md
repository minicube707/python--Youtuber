# 📘 Python Learning Projects – NeuralNine

A curated collection of Python projects and experiments based on high-quality tutorials from the **NeuralNine** YouTube channel.  
Each folder corresponds to a specific topic explored while learning Python and advanced programming concepts.

---

## 🎯 Objectives

- Learn Python from **intermediate to advanced levels**
- Explore **performance optimization**, **concurrency**, and **GPU computing**
- Practice **machine learning**, **cryptography**, and **network programming**
- Keep a clean and structured learning repository

---

## 📂 Repository Structure


> ⚠️ Folder names are kept as-is to reflect the original learning structure.

---

## 🧠 Topics & Resources

### 🔐 Cryptography & Security
- **Blockchain** → `Blockchain/`  
  https://www.youtube.com/watch?v=pYasYyjByKI
- **Hashing** → `Hashing/`  
  https://www.youtube.com/watch?v=i-h0CtKde6w
- **Password Security** → `Password/`  
  https://www.youtube.com/watch?v=rHTwjV1ORUQ
- **RSA Encryption** → `RSA/`  
  https://www.youtube.com/watch?v=D_PfV_IcUdA
- **QR Code** → `QRcode/`  
  https://www.youtube.com/watch?v=l4ugfcj7qrI

---

### ⚙️ Concurrency & Performance
- **Thread Pool** → `Thread Pool/`  
  https://www.youtube.com/watch?v=2Koubj0fF9U
- **Locking & Synchronizing** → `Locking & Synchronizing/`  
  https://www.youtube.com/watch?v=MbFSuk8yyjY
- **Garbage Collection** → `Garbade Collection/`  
  https://www.youtube.com/watch?v=pVGujarYk9w
- **Numba** → `Numba/`  
  https://www.youtube.com/watch?v=bZ5G-RZoE6Q
- **CuPy** → `CuPy/`  
  https://www.youtube.com/watch?v=lABJscdhsr8
- **NVIDIA GPU Acceleration** → `NVIDIA GPUs Accelerate/`  
  https://www.youtube.com/watch?v=QxlvD_yEgz8

---

### 🤖 Machine Learning & AI
- **PyTorch CNN** → `Pytorch CNN/`  
  https://www.youtube.com/watch?v=CtzfbUwrYGI
- **TensorFlow** → `Tensorflow/`  
  https://www.youtube.com/watch?v=bte8Er0QhDg

---

### 📊 Data Visualization
- **Matplotlib Animation** → `Matplolib Animation/`  
  https://www.youtube.com/watch?v=7RgoHTMbp4A
- **Matplotlib 3D** → `Matplotlib 3D/`  
  https://www.youtube.com/watch?v=fAztJg9oi7s

---

### 🧪 Python Internals & Extensions
- **Modules** → `Module/`  
  https://www.youtube.com/watch?v=GxCXiSkm6no
- **Cython** → `Cython/`  
  https://www.youtube.com/watch?v=Ic1oE6SEOBs
- **UV** → `UV/`  
  https://www.youtube.com/watch?v=l6WyNOIk0Ng

---

### 🌐 Networking & Web
- **Socket Programming** → `Socket/`  
  https://www.youtube.com/watch?v=YwWfKitB8aA
- **Web Crawler** → `Crawler/`  
  https://www.youtube.com/watch?v=m_3gjHGxIJc
- **Requests** → `Requests/`  
https://www.youtube.com/watch?v=Xi1F2ZMAZ7Q

---

### 🎮 Projects & Utilities
- **Hangman Game** → `Hangman/`  
  https://www.youtube.com/watch?v=5x6iAKdJB6U
- **Progress Bar** → `Progress Bar/`  
  https://www.youtube.com/watch?v=x1eaT88vJUA

---

## 🚀 How to Use This Repository

1. Choose a topic folder
2. Watch the linked NeuralNine video
3. Run and explore the code
4. Extend or refactor the implementation

---

## 📌 Notes

- This repository is **for educational purposes only**
- Code may differ slightly from the original tutorials
- All credit goes to **NeuralNine**

---

## ⭐ Acknowledgements

Thanks to **NeuralNine** for producing high-quality programming tutorials.
