...
def drawn_global_vision(win, width, rows, start, end, set_wall, set_path, set_open, set_closed):
    gap = width // rows
    
    for open in set_open:
        pygame.draw.rect(win, GREEN, (open[1]*gap, open[0]*gap,  gap, gap))
    
    for closed in set_closed:
        pygame.draw.rect(win, RED, (closed[1]*gap, closed[0]*gap,  gap, gap))
...
#Draw the window
def draw (win, rows, width, start, end, set_wall, set_path, set_open, set_closed):

    #Initialisation
    WIN.fill(WHITE)
    drawn_global_vision(win, width, rows, start, end, set_wall, set_path, set_open, set_closed)
...
def reconstruct_path(win, rows, width, start, end, set_wall, set_path, set_open, set_closed, came_from, current):
    set_path = set()
    while current in came_from:
        current = came_from[current]

        if current in set_open:
            set_open.remove(current)
        if current in set_closed:
            set_closed.remove(current)

        set_path.add(current)
        draw(win, rows, width, start, end, set_wall, set_path, set_open, set_closed)

    #Pygame event
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return set_path, set_open, set_closed, False, True
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    return set_path, set_open, set_closed, True, False
                
    return set_path, set_open, set_closed,True , False

def algorithm(win, width, rows, start, end, set_wall, run, stop):
    ...
    while not open_set.empty() and run and not stop:
        ...
        if current == end:
            set_path, set_open, set_closed, run , stop = reconstruct_path(win, rows, width, start, end, set_wall, {}, set_open, set_closed, came_from, current)
            return set_path, set_open, set_closed, run, stop
        ...
        for neigbors in list_neighbors:
            ...
            if temp_g__score < g_score[neigbors[0] * rows + neigbors[1]]:
                ...
                if neigbors not in open_set_hash:
                    ...
                    set_open.add(neigbors)

        list_neighbors.clear()

        draw(win, rows, width, start, end, set_wall, {}, set_open, set_closed)
        if current != start:
            if current in set_open:
                set_open.remove(current)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                return {}, set_open, set_closed, False, True
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    return {}, set_open, set_closed, True, False
                    
    return {}, set_open, set_closed, run, stop
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    set_path, set_open, set_closed, run, stop = algorithm(win, width, rows, start, end, set_wall, run , stop)
