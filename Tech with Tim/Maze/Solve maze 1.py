...
def drawn_global_vision(win, width, rows, start, end, set_wall, pathfinder):
    gap = width // rows
...
#Draw the window
def draw (win, rows, width, start, end, set_wall, pathfinder):

    #Initialisation
    WIN.fill(WHITE)
    drawn_global_vision(win, width, rows, start, end, set_wall, pathfinder)
...
def solve_maze(win, width, rows, start, end, set_wall):
    ...
    while current_node != end and run:
        ...
        draw(win, rows, width, start, end, set_wall, current_node)
...
def main (win , width):
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if (event.key == pygame.K_SPACE and start and end) or game:
                    ...
                    run = solve_maze(win, width, rows, start, end, set_wall)
                    end_time = time.time()
        ...
        draw(win, rows, width, start, end, set_wall, {})
