    ...
    #Up
        ...
        while (i <= rows - 1) and not placed_end:
            ...
            while (j <= rows - 1) and not placed_end:
                ...
                if state_neighbors == 7:
                    end_node = grid[j][current_node.total_rows - 1 - i]
                    end_node.make_end()
                    placed_end = True
...
def make_maze(grid, rows, win , width, nb_door):
    ...
    for _ in  range(nb_door):
        ...
        while reset:
            ...
            if grid[x][y].is_barrier() and not grid[x][y].is_start() and not grid[x][y].is_end():

                if (grid[x-1][y].is_barrier() and  grid[x+1][y].is_barrier()) or (grid[x][y-1].is_barrier() and  grid[x][y+1].is_barrier()):
                    grid[x][y].reset()
                    reset = False
                    draw(win, grid, rows, width)
...
def main(win, width):
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            while not finish:
                grid, start, end, finish,  run = make_maze(grid, rows, win , width, nb_door)
