...
y = x**3 -2*x**2 - 3*x +1 + (np.random.rand(40,1)-0.5)*2
plt.figure()
...
#Matrice X
order = int(input("Quelle est l'odre de la regression ? \n"))
X = np.ones(x.shape)
for i in range(1, order+1):
    X = np.hstack((np.sin(i*x),X))
    X = np.hstack((np.cos(i*x),X))

theta=np.random.rand(1+2*order,1)
...
#print("theta_initial vaut ",theta)
#print("theta_final vaut ",theta_final)
