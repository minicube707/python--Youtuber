...
# Python provides the multiprocessing module and the concurrent.futures.ProcessPoolExecutor class to simplify the creation and
# management of worker processes.
# By distributing work across several processes, programs can significantly reduce execution time and
# make better use of modern multi-core processors.

# Number of worker processes used in the examples
NUM_PROCESSES = 10
...
def return_value(seconds):
    """Sleep and return a sample value."""
    print(f"Sleeping for {seconds} second(s)...")
    time.sleep(seconds)
...
def run_two_processes():
    ...
    processes = [
        multiprocessing.Process(target=do_something),
        multiprocessing.Process(target=do_something),
    ]
...
def run_multiple_processes():
    ...
    print(f"\n=== {NUM_PROCESSES} manual processes ===")
    start = time.perf_counter()
