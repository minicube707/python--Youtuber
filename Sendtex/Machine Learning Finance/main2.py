...
def patternStorage():
    ...
    while (y < x):
        ...
        perfomanceAr.append(futureOutcome)

        y+=1

    patEnd = time.time()
    print(len(patternAr))
    print(len(perfomanceAr))
    print(patEnd - patStart)
...
def patternRecognition():
    ...
    if patFound:
        ...
        for eachPatt in plotPatAr:
            futurePoint = patternAr.index(eachPatt)

            if perfomanceAr[futurePoint] > patForRec[29]:
                pcolor = '#24bc00'
            ...
            predictedOutcomeAr.append(perfomanceAr[futurePoint])

            #plt.scatter(35, perfomanceAr[futurePoint], color=pcolor, alpha=.3)

        realOutcomeRange = allData[toWhat + 20: toWhat + 30]
        realAvgOutcome = sum(realOutcomeRange) / len(realOutcomeRange)
        realMouvement = percentChange(allData[toWhat], realAvgOutcome)
        ...
        if predictionAverage < 0:
            ...
            print(patForRec[29])
            print(realMouvement)
            if realMouvement < patForRec[29]:
                accuracyArray.append(100)
            else:
                accuracyArray.append(0)
        
        if predictionAverage > 0:
            print("rise preticted")
            print(patForRec[29])
            print(realMouvement)
            if realMouvement > patForRec[29]:
                accuracyArray.append(100)
...
def graphRawFX(date, bid, ask):
    ...
    ax1_2 = ax1.twinx()
    ax1_2.plot(date, (ask-bid), alpha=.3)
    ax1_2.fill_between(date, 0, (ask - bid), facecolor='g', alpha=.5)
...
#Show Graphic
graphRawFX(date, bid, ask)

dataLenght = len(bid)
print("Data Lenght: ", dataLenght)
...
while toWhat < dataLenght:
    ...
    samps += 1
    toWhat += 1
    acccuracyAverage = sum(accuracyArray) / len(accuracyArray)
    print("Backtested Accuracy is ", str(acccuracyAverage), "% after ", samps , " samples")